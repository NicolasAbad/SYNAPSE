...
describe('validateLoadedState — boundary defense', () => {
  ...
  test('accepts a correctly-shaped payload (124 keys)', () => {
    const good = createDefaultState();
    const result = validateLoadedState(good);
    expect(result).not.toBeNull();
    expect(Object.keys(result!).length).toBe(124);
  });
...
describe('round-trip type fidelity', () => {
  ...
  test('JSON.stringify drops actions + UI-local state is stripped → 124 file keys', async () => {
