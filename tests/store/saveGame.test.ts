...
describe('saveGame / loadGame round-trip', () => {
  ...
  test('round-trips all 123 GameState fields without loss', async () => {
...
describe('validateLoadedState — boundary defense', () => {
  ...
  test('accepts a correctly-shaped payload (123 keys)', () => {
    const good = createDefaultState();
    const result = validateLoadedState(good);
    expect(result).not.toBeNull();
    expect(Object.keys(result!).length).toBe(123);
  });
...
describe('round-trip type fidelity', () => {
  ...
  test('JSON.stringify drops actions + UI-local state is stripped → 123 file keys', async () => {
    // Store has GameState (123) + UIState (multi-key) + actions. saveToStorage strips
    // UI-local fields before persisting; JSON.stringify drops functions. Result:
    // persisted payload contains exactly the 123 GameState data fields.
    const storeSnapshot = useGameStore.getState();
    const storeKeyCount = Object.keys(storeSnapshot).length;
    expect(storeKeyCount).toBeGreaterThanOrEqual(123);
