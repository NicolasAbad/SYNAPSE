...
- **CODE-2:** MAX 200 lines per file, MAX 50 lines per function. One file = one purpose. Dependency flow: `config → engine → store → ui` (never backward). No circular imports.
  - **Exception A (file-level):** pure type definition files with >100 fields may exceed the 200-line cap IF the excess is entirely from documentation/section comments that aid audit and cross-reference. The exception must be documented in a file-level docstring citing this clause. As of Sprint 7.10.4 close, the only file under this exception is `src/types/GameState.ts` (~246 lines, 120 fields per GDD §32 post-7.10.4 `pendingOfflineSummary` add).
  - **Exception B (function-level):** pure object-literal constructors that mirror a type interface with >100 fields may exceed the 50-line function cap IF the body is exclusively one flat object literal (no logic, no branches). Splitting into sub-factories would add indirection without improving anything; the counterpart type exists under Exception A, and the function preserves the invariant `Object.keys(factory()).length === fieldCount`. As of Sprint 7.10.4 close, the only function under this exception is `createDefaultState()` in `src/store/gameStore.ts` (~186 lines, 120 fields).
