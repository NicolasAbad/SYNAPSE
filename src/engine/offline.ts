...
import { runUpgradeOfflineCapBonusHours } from './runUpgrades';
...
export function computeOfflineCapHours(state: Pick<GameState, 'upgrades' | 'resonanceUpgrades' | 'runUpgradesPurchased'>): number {
  ...
  // §15 time_dilation (+4h) and §21 sueno_profundo (+4h) both stack ADDITIVELY
  // on top of the max upgrade-set cap, then clamped at maxOfflineHours (16h).
  cap += resonanceOfflineCapBonusHours(state);
  cap += runUpgradeOfflineCapBonusHours(state);
...
export function detectTimeAnomaly(
  state: Pick<GameState, 'lastActiveTimestamp' | 'upgrades' | 'resonanceUpgrades' | 'runUpgradesPurchased'>,
  nowTimestamp: number,
